%-------------------------------------------------
% Perform sliding window analysis for the entire sample
...
% Data choices
data_folder = {'T1_VolumeMap_QC_Age_Edu_TIV_Sex_RES_Norm'};
...
for xls = 1:numel(xlsxfile)
    ...
    for w_wid = win_width
        for w_gap = win_gap
            ...
            disp(['+++++++++++++++++++++,', 'window_width = ', num2str(w_wid), ' overlap = ', num2str(1 - w_gap), ' is processing +++++++++++++++++++++'])
            ...
            for cir_num = 1:cir_max
                ...
                for j = 1:numel(VariableTable)
                    ...
                    for m = 1:numel(data_folder)
                        ...
                        subject = subject';
                        matdir = fullfile(outputdir, outputfolder{m}, outputcog{j}, 'Slide_win_Residual_Norm', ['Slide_win_', num2str(w_wid), '_', num2str(w_gap)], 'AllSubs', [num2str(min(cell2mat(Age))), '_', num2str(max(cell2mat(Age))), '_M_', num2str(mean(cell2mat(Age)))]);
                        mkdir(matdir);
